...
//ДЛЯ Коэна - 4х-битный код д/т-ки
...
{
	int code1 = computeOutCode(x1, y1); //код 1ой точки
	int code2 = computeOutCode(x2, y2); //2ой
	...
	while (true)
	{
		...
		else
		{
			...
			if (outCode & TOP) // снизу если , тогда двигаем 
			{
			...
			else if (outCode & LEFT) // А - слева - двигаем 
			{
				y = y1 + (y2 - y1) * (xMin - x1) / (x2 - x1);
				x = xMin;
			}

			if (outCode == code1) //1ая т-ка меняем ей бит
			{
				x1 = x;
				y1 = y;
...
// Бек - отсеч-е окном. Суть - скал-е произвед-е вектора ребра и внеш нормали. Смотрим по знаку, как пересекает.
	...
	else
	{
		...
		for (const auto& edge : edges)
		{
			...
			if (result != 0)
			{
				...
				if (result < 0)  // if <0, t(нач) = t, т.е. с внутренней на внеш сторону ребра 
				{
...
{
	if (P > 0) //линия ориентирована с видимой части области в невидимую.
	{
		double t = (double)Q / P;
		if (t > tOutput) return false; /* tInputMax > tOutputMin - снаружи */
		else if (t > tInput) tInput = t;
	}
	if (P < 0) // наоборот
	{
		double t = (double)Q / P;
		if (t < tInput) return false; /* tInputMax > tOutputMin - внутри */
		else if (t < tOutput) tOutput = t;
		else if (Q > 0) return false; /* line || границе */
	}
}


//pi = dx;  qi  = x1-xmin , xMax-xMin, ...

// t = qi/pi
//qi>0 - || границе
// qi<0 - за пределами

// pi>0 -> if t>t2 -> снаружи
//				if t>t0 ->t0=t
// pi<0 -> if t<t0 -> внутри
//				t<t2-> t2=t
...
void LiangBarsky(int x1, int y1, int x2, int y2, TGAImage& image)
{
	...
	else
	{
		...
		bool test2 = clipParameter(-dx, x1 - xMax, tInput, tOutput);
		bool test3 = clipParameter(dy, yMin - y1, tInput, tOutput);
		bool test4 = clipParameter(-dy, y1 - yMax, tInput, tOutput);*/

		bool test1 = clipParameter(dx, xMax - x1, tInput, tOutput);
		bool test2 = clipParameter(-dx, x1 - xMin, tInput, tOutput);
		bool test3 = clipParameter(dy, yMax - y1, tInput, tOutput);
		bool test4 = clipParameter(-dy, y1 - yMin, tInput, tOutput);
...
void liangAlgo()
{
	...
	LiangBarsky(100, 300, 500, 900, image);
	LiangBarsky(500, 900, 800, 100, image);
	LiangBarsky(800, 100, 100, 300, image);
